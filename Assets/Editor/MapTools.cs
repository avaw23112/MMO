...
public class MapTools
{
    ...
    {
        DataManager.Instance.Load(); //将数据加载到内存中

        Scene current = EditorSceneManager.GetActiveScene();//获取当前场景是什么

        string currentScene = current.name;//将场景名字记录下来
        //判断当前场景是否保存
        if (current.isDirty)
        {
        ...
        EditorUtility.DisplayDialog("提示", "传送点导出完成", "确定");//显示对话框
    ...
    {
        DataManager.Instance.Load(); //把原来的配置表内容加载一遍
        ...
        foreach (var map in DataManager.Instance.Maps) //遍历每一张地图
        {
            string sceneFile = "Assets/Levels/" + map.Value.Resource + ".unity";
            if (!System.IO.File.Exists(sceneFile))  // 判断该路径有没有该文件
            {
                Debug.LogWarningFormat("Scene{0} not existed!", sceneFile);
                continue;
            }
