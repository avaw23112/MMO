...
namespace Services
{
    class UserService : Singleton<UserService>, IDisposable
    {
        ...
        internal void SendCharacterDelete(NCharacterInfo characterInfo)
        {
            Debug.LogFormat("UserDeleteCharacterRequest::TId:{0} name :{1} class:{2}", characterInfo.Id, characterInfo.Name, characterInfo.Class);
            NetMessage message = new NetMessage();
            message.Request = new NetMessageRequest();
            message.Request.deleteChar = new UserDeleteCharacterRequest();
            message.Request.deleteChar.characterId = characterInfo.Id;
            message.Request.deleteChar.Name = characterInfo.Name;
            if (this.connected && NetClient.Instance.Connected)
            {
                this.pendingMessage = null;
                NetClient.Instance.SendMessage(message);
